...
export class CanvasBackground {
    constructor(canvasId) {
        ...
        this.dragonSegments = [];
        for (let i = 0; i < 40; i++) {
            this.dragonSegments.push({ x: 0, y: 0 });
        }
        this.splashes = [];
    ...
    resetHeroAnchors(width, height) {
        this.mouseX = width / 2;
        this.mouseY = height / 2;
        for (let i = 0; i < this.dragonSegments.length; i++) {
            this.dragonSegments[i].x = this.mouseX;
            this.dragonSegments[i].y = this.mouseY;
        }
        this.splashes = [];
    }

    resize() {
        const { width, height } = this.getHeroSize();
        this.canvas.width = width;
        this.canvas.height = height;
        this.resetHeroAnchors(width, height);
        this.initParticles();
    }
