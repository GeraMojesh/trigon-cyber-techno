...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawDonut(canvas) {
        ...
        const data = [
            { value: 42, color: '#FF2D78' },
            { value: 28, color: '#7B6FF0' },
    ...
    function initThreatMap(canvas) {
        ...
        const cities = [
            ...
            { name: 'London',      lon: -0.1, lat: 51.5, color: '#7B6FF0', target: false },
            { name: 'Moscow',      lon: 37.6, lat: 55.7, color: '#FF2D78', target: true },
            { name: 'Beijing',     lon: 116,  lat: 39.9, color: '#FF2D78', target: true },
            { name: 'Tokyo',       lon: 139,  lat: 35.7, color: '#F97316', target: false },
            { name: 'Mumbai',      lon: 72.8, lat: 19,   color: '#3B82F6', target: false },
            { name: 'São Paulo',   lon: -46,  lat: -23.5,color: '#28c840', target: false },
            { name: 'Lagos',       lon: 3.4,  lat: 6.5,  color: '#F97316', target: false },
            { name: 'Sydney',      lon: 151,  lat: -33.8,color: '#7B6FF0', target: false },
            { name: 'Dubai',       lon: 55.3, lat: 25.2, color: '#3B82F6', target: false },
            { name: 'Singapore',   lon: 103.8,lat: 1.3,  color: '#28c840', target: false },
            { name: 'Kyiv',        lon: 30.5, lat: 50.5, color: '#FF2D78', target: true },
        ];

        // Attack routes [srcIdx, dstIdx, color]
        const attacks = [
            { src: 2, dst: 0, color: '#FF2D78' }, // Moscow → NY
            { src: 3, dst: 1, color: '#FF2D78' }, // Beijing → London
            { src: 3, dst: 0, color: '#F97316' }, // Beijing → NY
            { src: 2, dst: 11, color: '#FF0055'}, // Moscow → Kyiv
            { src: 3, dst: 9, color: '#7B6FF0' }, // Beijing → Dubai
            { src: 0, dst: 7, color: '#3B82F6' }, // NY → Lagos
        ];
        ...
        function draw() {
            ...
            /*
            ctx.strokeStyle = 'rgba(123,111,240,0.06)';
            ctx.lineWidth = 0.5;
            ...
            landMasses.forEach(poly => {
                ...
                ctx.fill();
                ctx.strokeStyle = 'rgba(123,111,240,0.3)';
                ctx.lineWidth = 0.8;
                ctx.stroke();
            });
            */
    ...
    const feedData = [
        { time: '22:41', type: 'Phishing', src: '185.220.x.x', sev: 'HIGH', status: 'BLOCKED', color: '#FF2D78' },
        { time: '22:39', type: 'Port Scan', src: '103.14.x.x', sev: 'LOW', status: 'LOGGED', color: '#3B82F6' },
        { time: '22:37', type: 'Brute Force', src: '91.193.x.x', sev: 'MED', status: 'BLOCKED', color: '#F97316' },
        { time: '22:35', type: 'Malware', src: '45.147.x.x', sev: 'HIGH', status: 'QUARANTINE', color: '#FF2D78' },
        { time: '22:33', type: 'SQL Inject', src: '194.165.x.x', sev: 'CRIT', status: 'BLOCKED', color: '#FF0055' },
        { time: '22:30', type: 'DDoS Pulse', src: '198.98.x.x', sev: 'MED', status: 'MITIGATED', color: '#F97316' },
        { time: '22:28', type: 'XSS Probe', src: '89.248.x.x', sev: 'LOW', status: 'LOGGED', color: '#3B82F6' },
    ];
    ...
    const sevColors = { LOW: '#3B82F6', MED: '#F97316', HIGH: '#FF2D78', CRIT: '#FF0055' };
